...
Add `cburnett` to the `pieceSets` catalog: the vendored default artwork is now also selectable as a named preset alongside rhosgfx, kiwenSuwi, chessnut, spatial, and celtic. Omitting `pieceSet` keeps rendering the same Cburnett artwork; the separate internal default module is gone (one embedded copy instead of two).
